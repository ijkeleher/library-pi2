...
"""
    ...
    def search(self, query):
        ...
        q = "%" + query + "%"
        cursor.execute(
            "SELECT * FROM Book WHERE UPPER(Title) LIKE UPPER(%s) OR UPPER(Author) LIKE UPPER(%s) OR UPPER(PublishedDate) LIKE UPPER(%s) OR ISBN LIKE %s",
            [
                q,
                q,
                q,
    ...
    def borrow(self, book_id, calender):
        ...
        if data is not None:
            ...
            cursor.execute(
                "INSERT INTO BookBorrowed (LMSUserID, BookID, Status, BorrowedDate, EventID) VALUES (%s, %s, %s, %s, %s)", [
                    1, book_id, "borrowed", datetime.now().strftime('%Y-%m-%d'), event_id])
            self.__conn.commit()
    ...
    def return_book(self, book_id, calender):
        ...
        cursor = self.__conn.cursor()
        cursor.execute(
            "SELECT EventID, BookBorrowedID FROM BookBorrowed WHERE BookID=%s AND Status=%s", [
                book_id, "borrowed"])
        ...
        if data is not None and data[0] is not None:
            ...
            cursor.execute(
                "UPDATE BookBorrowed SET Status=%s WHERE BookBorrowedID=%s", [
                    "returned", book_borrow_id])
            self.__conn.commit()
                ...
                while True:
                    ...
                    if menu is 'main':
                        ...
                        if books is '':
                            ...
                            for book in books:
                                response += str(book[1]).ljust(32) + " by " + str(book[2]).ljust(
                                    20) + " ISBN: " + str(book[4]) + "\t Book ID: " + str(book[0]) + "\n"
                        ...
                        print("Attempting to borrow " + str(book_id) +
                              " which is " + str(type(book_id)))
